"""Small encoder-decoder CNN for relative monocular depth.

This is an architectural baseline only. Randomly initialized weights do not
produce useful depth; the model must be trained before application use.
"""
...
class ConvBlock(nn.Module):
    """Two convolution layers with batch-size-independent normalization."""
...
class TinyDepthNet(nn.Module):
    """Compact U-Net-style CNN producing inverse depth and confidence.

    The first output channel is normalized relative inverse depth: larger
    values indicate points closer to the camera. The second output channel is
    an independently learned confidence estimate.
    """
    ...
    def __init__(self, in_channels: int = 3, base_channels: int = 24) -> None:
        ...
        self.output_head = nn.Conv2d(base_channels, 2, kernel_size=1)

    def encode(self, inputs: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        """Return high-to-low-resolution features for future shared heads."""
    ...
    def forward(self, inputs: Tensor) -> tuple[Tensor, Tensor]:
        ...
        prediction = self.output_head(decoded_1)
        inverse_depth = torch.sigmoid(prediction[:, 0:1])
        confidence = torch.sigmoid(prediction[:, 1:2])
        return inverse_depth, confidence
